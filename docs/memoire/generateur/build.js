...
add(H1("13. Rôle de chaque élément de la commande"),
  ...
  ...TAB("Éléments de conception", ["Élément", "Nature", "Rôle"], [
    ...
    ["Banc de tests (tests/)", "Code C++ PC", "Exécute le firmware réel sans matériel ; 200 vérifications"],
    ["simulation_cycle.cpp", "Code C++ PC", "Firmware réel + modèle thermique illustratif → exemple de cycle"],
    ["Commande_Sechoir_Hybride_corrige.slx", "Modèle Simulink", "Simulation : FSM + conversion de puissance + modèle thermique"],
    ["completer_fsm_sechoir.m", "Script MATLAB", "Construit automatiquement le chart Stateflow à l'identique du firmware"],
  ...
  ...TAB("Blocs du modèle", ["Bloc", "Rôle"], [
    ...
    ["Blocs Constant / Step", "Consignes, paramètres et scénarios (pression, humidité, boutons, défauts)"],
    ["Scopes", "Observation des signaux (T_sec, palier, états, sorties)"],
  ], [3.5, 6.5]),
);
...
add(H1("14. Modèle Simulink / Stateflow et guide de simulation"),
  ...
  ...N([
    "Ouvrir `matlab/Commande_Sechoir_Hybride_corrige.slx` (chart d'origine + corrections physiques).",
    "Dans le dossier `matlab/`, exécuter `completer_fsm_sechoir('Commande_Sechoir_Hybride_corrige')`.",
    "Si la sortie affiche `[HIERARCHIE INCORRECTE]` : glisser-déposer les états signalés dans leur boîte parente, régler `EN_CYCLE` en décomposition « AND (parallel) », relancer le script.",
    "Clic droit sur le chart → « Update Chart ».",
    "Câbler les deux nouvelles entrées `Btn_Stop` et `Btn_Rearm` (Constant à 0 pour commencer).",
    "Régler les entrées et paramètres du tableau suivant, puis simuler.",
  ]),
  H2("14.3 Réglages indispensables avant de simuler"),
  ...TAB("Réglages avant simulation", ["Entrée / paramètre", "Réglage conseillé", "Pourquoi"], [
    ["Flame", "Flame = (V_H2 OU V_But), retardé de 1 s (Transport Delay)", "Sinon, une constante 1 déclenche l'urgence « flamme parasite » quand le gaz est fermé, et une constante 0 provoque des échecs d'allumage"],
    ["Tps_Prolongation", "1800 (secondes)", "Durée de prolongation proposée"],
    ["H_sec", "Modèle de séchage simple, ou Step décroissant", "Sinon le critère d'humidité ne se déclenche jamais"],
    ["Press_H2", "Constant 8 (bar) ; Step pour tester la bascule", "H2 disponible"],
    ["MQ8_H2, MQ6_But", "Constant 50 ; Step > 350 pour tester l'urgence", "Pas de fuite"],
    ["AU_Manuel", "0", "Pas d'arrêt d'urgence"],
    ["Btn_Start", "Impulsion à t = 10 s", "Démarrage du cycle"],
    ["Mode_Auto", "1", "Arbitrage automatique"],
    ["Pas de calcul", "Fixe, 0,1 s au plus", "Réactivité de la sécurité flamme"],
  ], [2.4, 3.6, 4]),
  ...
  ...TAB("Scénarios de simulation et résultats attendus", ["N°", "Scénario", "Réglage", "Résultat attendu"], [
    ...
    ["2", "Démarrage solaire", "T_amb ≥ 35 (T_cap_est ≥ 55)", "MODE_SOLAIRE, aucune vanne ouverte"],
    ["3", "Bascule H2 → GPL", "Press_H2 : Step 8 → 0,5 bar à 30 min", "Purge, rallumage GPL au palier conservé"],
    ["4", "Retour au solaire", "T_amb : rampe montante", "Extinction normale + post-purge 120 s"],
    ["5", "Échecs d'allumage", "Flame forcée à 0", "3 essais espacés de purges → ERREUR_COMBUSTION"],
    ["6", "Pertes de flamme", "Flame à 0 deux fois en régulation", "Relance, puis URGENCE (cause 4)"],
    ...
    ["11", "Palier imposé", "Regul_Auto = 0, Palier_Impose = 2", "Palier 33 % quelle que soit T_sec"],
    ["12", "Surchauffe", "Palier imposé 100 % + gain thermique élevé", "T_sec ≥ 90 °C → SECHAGE_TERMINE"],
  ], [0.6, 2.4, 3.3, 3.7]),
  ...
  NOTE("Important", "Le script `completer_fsm_sechoir.m` n'a pas encore été exécuté dans une vraie session MATLAB. Sa syntaxe et celle des 37 libellés qu'il écrit dans le chart ont été vérifiées par analyse statique (chapitre 15), mais pas les appels à l'API Stateflow. Le premier essai doit être considéré comme une validation."),
);
...
add(H1("15. Validation"),
  ...
  ...TAB("Cas de test (200 vérifications, toutes réussies)", ["Cas", "Exigence vérifiée"], [
  ...
  P("Un banc qui réussit tout ne prouve rien s'il ne sait pas détecter d'erreur. Le **test par mutation** consiste à introduire volontairement un défaut dans le code (une « mutation ») et à vérifier que le banc échoue [17], [18]. Onze mutations, chacune désactivant une règle de la version 3 (tolérer la 2e perte de flamme, ignorer la flamme parasite, supprimer le repère, allumer toujours à 100 %, etc.), ont **toutes** été détectées ; le code restauré repasse à 200/200."),
  H2("15.3 Vérification du modèle Stateflow"),
  P("Sans MATLAB disponible, le script de construction et les 37 libellés qu'il génère (9 états, 28 transitions) ont été analysés par **MISS_HIT** [29], un analyseur statique du langage MATLAB : syntaxe correcte. Il a aussi été vérifié que chaque variable utilisée est déclarée et qu'aucune entrée n'est écrite."),
  H2("15.4 Ce qui reste à valider"),
  ...L([
    "Exécution du script Stateflow et simulation dans MATLAB/Simulink.",
...
add(H1("16. Logiciels et outils utilisés"),
  ...TAB("Logiciels, bibliothèques et outils", ["Outil", "Usage dans le projet", "Source"], [
    ...
    ["GCC / g++ et GNU Make", "Compilation native du banc de tests et de la simulation de cycle", "[34]"],
    ["MATLAB, Simulink, Stateflow", "Modèle de la commande et simulation", "[31]"],
