...
add(H1("14. Modèle Simulink / Stateflow et guide de simulation"),
  ...
  NOTE("Important", "Les scripts ont été vérifiés par analyse statique (chapitre 15). Les premiers essais sous MATLAB R2025b ont validé la création des données, de la hiérarchie et des transitions ; le modèle construit à partir de zéro compile et **les douze scénarios sont validés contre le firmware** (§15.3)."),
);
...
add(H1("15. Validation"),
  ...
  P("**Validation croisée sous MATLAB R2025b** : les douze scénarios ont été simulés avec le modèle Stateflow puis comparés à la référence du firmware (`comparer_scenario`). Tous les changements d'état arrivent dans le même ordre et aux mêmes instants :"),
  ...TAB("Validation croisée Simulink / firmware (12 scénarios)", ["N°", "Scénario", "Changements d'état (Simulink)", "Écart max"], [
    ["1", "Démarrage à froid H2", "H2 10,1 s", "0,1 s"],
    ["2", "Démarrage solaire", "SOLAIRE 10,1 s", "0,1 s"],
    ["3", "Bascule H2 → GPL", "H2 10,1 s → GPL 1250,1 s", "0,1 s"],
    ["4", "Retour au solaire", "H2 10,1 s → SOLAIRE 2160,1 s", "0,1 s"],
    ["5", "Échecs d'allumage", "H2 → ERREUR 382,1 s → H2 700,1 s", "0,1 s"],
    ["6", "Pertes de flamme", "H2 → URGENCE 700,0 s → ATTENTE 900,0 s", "0,1 s"],
    ["7", "Fuite H2", "H2 → URGENCE 1500 s → ATTENTE 1900 s → H2 2000,1 s", "0,1 s"],
    ["8", "Fin par humidité", "DEMANDE 7210,1 s → TERMINÉ 7510 s → ATTENTE 7810 s", "0,1 s"],
    ["9", "Prolongation acceptée", "DEMANDE 7210,1 s → PROLONG. 7300,1 s → DEMANDE 9100,1 s → TERMINÉ 9400 s", "0,1 s"],
    ["10", "Humidité FIXE", "DEMANDE 5410,1 s → TERMINÉ 5710 s → ATTENTE 6010 s", "0,1 s"],
    ["11", "Palier imposé 33 %", "TERMINÉ (90 °C) 1944,9 s → ATTENTE 2244,9 s", "0,1 s"],
    ["12", "Surchauffe (100 % imposé)", "TERMINÉ (90 °C) 632,4 s → ATTENTE 932,4 s", "0,6 s"],
  ], [0.6, 2.6, 5, 1.2]),
  P("L'écart de 0,1 s correspond au pas de calcul du chart. Les instants de changement de palier s'écartent au plus d'environ 4 s par cycle de veille (14 s après une heure dans le scénario 1) : le firmware lit la sonde T_sec une fois par seconde avec une conversion asynchrone du DS18B20, alors que Simulink la lit tous les 0,1 s ; ce retard de mesure prolonge légèrement chaque veille. La séquence des états reste identique : **le modèle Stateflow reproduit le programme Arduino**."),
  ...
  ...L([
    "Identification du modèle thermique et du modèle de séchage sur le prototype (les paramètres actuels sont ceux du chapitre 3 et un modèle de séchage illustratif).",
